...
class Scribbler: public Robot {
	 ...
	 * @param int freq2 - an opitional second frequency, defaults to 0
	 */
	void beep(double duration, int freq, int freq2 = 0);

	/**
	 * Allows used set a single byte value stored on the robot.
	...
	/**
	 * A general purpose get function which will query teh specified
	 ...
	 *						
	 * @return The return value is a void * pointer, it is up to the caller
	 * to know which return value they are expecting adn to cast accordingly.
	 */
	 ...
	 *
	 * @param value - specifiy an intensity for the rear led.
	 ...
	 *
	 * @param direction - 1 for fluke-foward and 0 for scribbler-forward
	 */
	...
	/**
	 * Used to configure the blob tracking freature within the robot,
	 ...
	 * @return unsigned char array containing the image infromation.
	 */
	...
	/**
	 * This returns informat about the robot's dongle, firmware,
	...
	/**
	 * @param An opitional vector used to specify desired byte indexes.
	 * @return This function returns either all 8 bytes stored on the robot,
	 * or a set of the data based on the input vector. 
	 *
	 * Example: A vector containining 1, 2, 3 will return the data bytes
	 ...
	 * is not useable since some commands use 0 as a valid transmission
	 * value.
	 */
	 ...
	 *
	 * @param message_header - teh message header for the desired get command.
	 ...
	 * @param sensor - default value 1, set to 0 to not read the
	...
	//camera settings, the values are hardcoded into the constructor
