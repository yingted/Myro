...
;;; Modifications have been made to fit Chez Scheme 
...
(define-record :char-set (s))
(define make-char-set make-:char-set)
(define char-set? :char-set?)
(define char-set:s :char-set-s)
...
(define (%default-base maybe-base proc)
  (if (pair? maybe-base)
	...
	(if (null? tail)
		...
		(error '%default-base 
			   "BASE-CS parameter not a char-set: ~s ~s" proc bcs))
	    (error '%default-base
		  "Expected final base char set -- too many parameters: ~s ~s"
		   proc maybe-base)))
...
(define (%char-set:s/check cs proc)
  (let lp ((cs cs))
	...
	(lp (error '%char-set:s/check "Not a char-set: ~s ~s" cs proc)))))
...
(define (%ucs-range->char-set! lower upper error? bs proc)
  ...
  (if (and (< lower upper) (< 256 upper) error?)
      (error '%ucs-range->char-set! 
			 "Requested UCS range contains unavailable characters -- this implementation only supports Latin-1: ~s ~s ~s"
	     proc lower upper))
