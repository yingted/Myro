...
class Picture(GraphicsObject):
	...
	def __init__(self, p, pixmap):
		GraphicsObject.__init__(self, [])
		self.anchor = p.clone()
		self.imageId = Picture.idCount
		Picture.idCount = Picture.idCount + 1
		if "PhotoImage" == pixmap.__class__.__name__:
			self.img = pixmap
		elif type(pixmap) == type(""):
	...
	def clone(self):
		...
		other = Picture(self.anchor, imgCopy)
		other.config = self.config.copy()
		return other
