...
(define-datatype expression expression?
  ...
  (try2-exp
    (body expression?)
    (var symbol?)
    (except-exps (list-of expression?)))
  (raise-exp
    (exp expression?))
  (spawn-exp
    (name string?)
    ...
    (cond
	 ...
	 (lambda (v1)
      ...
      ((try2? datum)
       (parse (cadr datum)
	 (lambda (v1)
	   (parse-all (cdr (cddddr datum))
	     (lambda (v2)
	       (k (try2-exp v1 (cadddr datum) v2)))))))
      ((raise? datum)
       (parse (cadr datum)
	 (lambda (v)
	   (k (raise-exp v)))))
...
(define define-syntax? (tagged-list 'define-syntax > 2))
(define begin? (tagged-list 'begin > 1))
(define lambda? (tagged-list 'lambda > 2))
(define try? (tagged-list 'try = 4))
(define try2? (tagged-list 'try > 5))
  ...
  (lambda (x)
    (and (symbol? x)
	 (memq x '(quote quasiquote lambda if set! define begin
		    ...
		    try raise spawn lock acquire release)))))
