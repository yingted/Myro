...
(define apply-parser-cont
  (lambda (k value)
    (record-case k
	   ...
	   (apply-parser-cont k `(cons ,v1 ,value)))
       (expand-quasi-2 (datum handler v2 k)
	   (expand-quasiquote (cdr datum) handler 'expand-quasi-1 value v2 k))
       (expand-quasi-3 (datum k)
	   (apply-parser-cont k `(append ,(cadr (car datum)) ,value)))
       (expand-quasi-4 (k)
	   (apply-parser-cont k `(list->vector ,value)))
       (parse-all-1 (a k)
           (apply-parser-cont k (cons a value)))
       (parse-all-2 (datum-list handler k)
   	   (parse-all (cdr datum-list) handler (make-cont 'parse-all-1 value k)))
       (parse-1 (v1 k)
           (apply-parser-cont k (app-exp v1 value)))
       (parse-2 (datum handler k)
	   (parse-all (cdr datum) handler (make-cont 'parse-1 value k)))
       (parse-3 (k)
	   (apply-parser-cont k (raise-exp value)))
       (parse-4 (datum k body cvar cexps)
           (let ((cvar (catch-var (caddr datum))))
	     (apply-parser-cont k (try-catch-finally-exp body cvar cexps value))))
       (parse-5 (datum handler k body cvar)
           (parse-all (finally-exps (cadddr datum)) handler (make-cont 'parse-4 datum k body cvar value)))
       (parse-6 (datum handler k cvar)
           (parse-all (catch-exps (caddr datum)) handler (make-cont 'parse-5 datum handler k value cvar)))
       (parse-7 (k body)
	   (apply-parser-cont k (try-finally-exp body value)))
       (parse-8 (datum handler k)
	   (parse-all (finally-exps (caddr datum)) handler (make-cont 'parse-7 k value)))
       (parse-9 (datum k body cvar)
	   (let ((cvar (catch-var (caddr datum))))
	     (apply-parser-cont k (try-catch-exp body cvar value))))
       (parse-10 (datum handler datum k cvar)
	   (parse-all (catch-exps (caddr datum)) handler (make-cont 'parse-9 datum k value cvar)))
       (parse-11 (datum)
	   (if (proper-list? (cadr datum))
	       (apply-parser-cont k (lambda-exp (cadr datum) value))
	       (apply-parser-cont k (mu-lambda-exp (head (cadr datum)) (last (cadr datum)) value))))
       (parse-12 (k)
	   (apply-parser-cont k (begin-exp value)))
       (parse-13 (k datum)
	   (apply-parser-cont k (define-exp (cadr datum) value)))
       (parse-14 (k datum)
	   (apply-parser-cont k (assign-exp (cadr datum) value)))
       (parse-15 (k v1 v2)
	   (apply-parser-cont k (if-exp v1 v2 value)))
       (parse-16 (datum handler k v1)
	   (parse (cadddr datum) handler (make-cont 'parse-15 k v1 value)))
       (parse-17 (datum handler k)
	   (parse (caddr datum) handler (make-cont 'parse-16 datum handler k value)))
       (parse-18 (k v1)
	   (apply-parser-cont k (if-exp v1 value (lit-exp #f))))
       (parse-19 (datum handler k)
	   (parse (caddr datum) handler (make-cont 'parse-18 k value)))
       (parse-20 (handler k)
	   ...
	   (if (list? value)
	       ...
	       (apply-parser-cont k (value datum))))
...
(define apply-parser-cont2
  (lambda (k datum tokens-left)
    (record-case k
	   ...
	   (parse datum handler (make-cont 'print-parsed-sexps tokens-left)))
       (parse-string (test-handler)
	   (parse datum test-handler (make-cont 'init)))
...
(define expand-once
  (lambda (datum handler k)
    (lookup-value (car datum) macro-env handler (make-cont 'expand-once datum handler k datum))))
...
(define process-macro-clauses
  (lambda (clauses datum handler k)
	...
	(unify-patterns left-pattern datum (make-cont 'process-macro-clauses right-pattern k clauses datum handler))))))
...
(define parse-string
  (lambda (string)
    (read-datum string test-handler (make-cont 'parse-string test-handler))))

(define parse
  (lambda (datum handler k)
    (cond
      ((literal? datum) (apply-parser-cont k (lit-exp datum)))
      ((quote? datum) (apply-parser-cont k (lit-exp (cadr datum))))
      ((quasiquote? datum)
       (expand-quasiquote (cadr datum) handler (make-cont 'parse-21 handler k)))
      ((unquote? datum) (handler (format "misplaced ~s" datum)))
      ((unquote-splicing? datum) (handler (format "misplaced ~s" datum)))
      ((symbol? datum) (apply-parser-cont k (var-exp datum)))
      ((syntactic-sugar? datum)
       (expand-once datum handler (make-cont 'parse-20 handler k)))
      ((if-then? datum)
       (parse (cadr datum) handler (amke-cont 'parse-19 datum handler k))) 
      ((if-else? datum)
       (parse (cadr datum) handler (make-cont 'parse-17 datum handler k)))
      ((assignment? datum)
       (parse (caddr datum) handler (make-cont 'parse-14 k datum)))
      ((define? datum)
       (if (mit-define? datum)
	 (parse (mit-define-transformer datum) handler k)
	 (parse (caddr datum) handler (make-cont 'parse-13 k datum))))
      ((define-syntax? datum)
       (apply-parser-cont k (define-syntax-exp (cadr datum) (cddr datum))))
      ((begin? datum)
       (parse-all (cdr datum) handler (make-cont 'parse-12 k)))
      ((lambda? datum)
       (parse (cons 'begin (cddr datum)) handler (make-cont 'parse-11 datum)))
      ((try? datum)
       (cond
	 ((= (length datum) 2)
	  ;; (try <body>)
	 ...
	 ((and (= (length datum) 3) (catch? (caddr datum)))
	  ;; (try <body> (catch <var> <exp> ...))
	  (parse (try-body datum) handler (make-cont 'parse-10 datum handler datum k cvar)))
	 ((and (= (length datum) 3) (finally? (caddr datum)))
	  ;; (try <body> (finally <exp> ...))
	  (parse (try-body datum) handler (make-cont 'parse-8 datum handler k)))
	 ((and (= (length datum) 4) (catch? (caddr datum)) (finally? (cadddr datum)))
	  ;; (try <body> (catch <var> <exp> ...) (finally <exp> ...))
	  (parse (try-body datum) handler (make-cont 'parse-6 datum handler k cvar)))
	 (else (handler (format "bad try syntax: ~s" datum)))))
      ((raise? datum)
       (parse (cadr datum) handler (make-cont 'parse-3 k)))
      ((application? datum)
       (parse (car datum) handler (make-cont 'parse-2 datum handler k)))
...
(define parse-all
  (lambda (datum-list handler k)
    (if (null? datum-list)
	(apply-parser-cont k '())
	(parse (car datum-list) handler (make-cont 'parse-all-2 datum-list handler k)))))

(define expand-quasiquote
  (lambda (datum handler k)
    (cond
      ((vector? datum)
       (expand-quasiquote (vector->list datum) handler (make-cont 'expand-quasi-4 k)))
      ((not (pair? datum)) (apply-parser-cont k `(quote ,datum)))
      ;; doesn't handle nested quasiquotes yet
      ((quasiquote? datum) (apply-parser-cont k `(quote ,datum)))
      ((unquote? datum) (apply-parser-cont k (cadr datum)))
      ((unquote-splicing? (car datum))
       (if (null? (cdr datum))
	 (apply-parser-cont k (cadr (car datum)))
	 (expand-quasiquote (cdr datum) handler (make-cont 'expand-quasi-3 datum k))))
      (else
       (expand-quasiquote (car datum) handler (make-cont 'expand-quasi-2 datum handler v2 k))))))
...
(define parse-file
  (lambda (filename)
    (scan-input (read-content filename) test-handler (make-cont 'parse-file handler))))
...
(define print-parsed-sexps
  (lambda (tokens handler)
    (if (token-type? (first tokens) 'end-marker)
      'done
      (read-sexp tokens handler (make-cont 'print-parsed-sexps-2 handler)))))
