# File:        ikinematics.py
# Description: Contains Inverse kinematics for Robonova legs
...
def rotYawPitchLeft(a):
  ...
  ca = cos(a)
  sa = sin(a)
  r = array([[ca, sa/sqrt(2), sa/sqrt(2)], 
             [-sa/sqrt(2), .5*(1+ca), .5*(-1+ca)], 
             [-sa/sqrt(2), .5*(-1+ca), .5*(1+ca)]])
  return r
...
def trPosition6D(p):
  ...
  cwz = cos(wz)
  swz = sin(wz)
  tr = array([[cwy*cwz,  swx*swy*cwz-cwx*swz, cwx*swy*cwz+swx*swz, x], 
              [cwy*swz, swx*swy*swz+cwx*cwz,  cwx*swy*swz-swx*cwz, y], 
              [-swy, swx*cwy, cwx*cwy, z], 
              [0., 0., 0., 1.]])
  return tr
...
def ikineLegs(*args):
  ...
  # Compute: hipYawPitch, hipRoll, hipPitch, kneePitch, anklePitch, ankleRoll
  ...
  qT = array([hipYawPitch, hipRoll, hipPitch, 
